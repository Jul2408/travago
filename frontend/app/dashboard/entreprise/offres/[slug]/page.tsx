...
export default function JobApplicationsPage() {
    ...
    return (
        <div className="space-y-10">
            ...
            <div className="bg-white rounded-[3rem] p-10 border border-blue-50 shadow-sm">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-8">
                    <div className="flex items-start space-x-6">
                        {offer?.company_detail?.logo ? (
                            <div className="relative w-24 h-24 rounded-[2rem] overflow-hidden border border-blue-50 shadow-xl shadow-blue-100">
                                <Image
                        ...
                        <div>
                            <h1 className="text-3xl font-black text-slate-900 tracking-tight uppercase mb-2">{offer?.title}</h1>
                            <div className="flex items-center gap-6 text-[10px] font-black text-slate-400 uppercase tracking-widest">
                                <span className="flex items-center"><MapPin size={14} className="mr-2 text-blue-400" /> {offer?.location}</span>
                                <span className="flex items-center"><Clock size={14} className="mr-2 text-blue-400" /> {offer?.contract_type}</span>
                                <span className="flex items-center"><Users size={14} className="mr-2 text-blue-400" /> {applications.length} Candidatures</span>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div className="grid grid-cols-1 gap-6">
                <h2 className="text-xl font-black text-slate-900 uppercase tracking-tighter">Candidats Correspondants</h2>

                {applications.length === 0 ? (
                    <div className="text-center py-20 bg-slate-50 rounded-[3rem] border-2 border-dashed border-slate-200">
                        <Users size={48} className="mx-auto text-slate-300 mb-4" />
                        <h3 className="text-xl font-black text-slate-400 uppercase">Aucune candidature pour le moment</h3>
                    </div>
                ) : (
                    applications.sort((a, b) => b.matching_score - a.matching_score).map((app) => {
                        ...
                        return (
                            <div key={app.id} className="group bg-white rounded-[2.5rem] p-8 border border-blue-50 shadow-sm hover:shadow-2xl transition-all">
                                <div className="flex flex-col lg:flex-row lg:items-center gap-10">
                                    ...
                                    <div className="flex items-center space-x-6 lg:w-1/3">
                                        ...
                                        <div>
                                            <div className="flex items-center space-x-2 mb-1">
                                                <h4 className="font-black text-slate-900 tracking-tight text-lg">{fullName}</h4>
                                                {app.candidate_detail.is_verified && <ShieldCheck size={16} className="text-blue-600" />}
                                            </div>
                                            <p className="text-[10px] font-black text-blue-400 uppercase tracking-widest">{app.candidate_detail.title || 'Candidat'}</p>
                                        </div>
                                    </div>

                                    {/* IA Compatibility Score */}
                                    <div className="flex flex-col items-center justify-center flex-1 px-10 border-x border-slate-50 text-center">
                                        <div className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-3">Score de Matching</div>
                                    ...
                                    <div className="flex items-center justify-end gap-4 lg:w-1/3">
                                        <Link
                                            ...
                                            className="p-4 bg-slate-50 text-slate-400 rounded-2xl hover:bg-blue-50 hover:text-blue-600 transition-all block"
                                        >
                                            <Eye size={20} />
                                        </Link>
                                        <button
                                            ...
                                            className="bg-slate-900 text-white px-8 py-4 rounded-2xl font-black text-[10px] uppercase tracking-widest hover:bg-blue-600 transition-all flex items-center shadow-lg shadow-slate-100 disabled:opacity-50 disabled:cursor-not-allowed"
                                        >
